...
def main():
    ...
    render_parser.add_argument("--texture", required=True, help="Path to the block texture")
    ...
    if args.command == "render":
        ...
        if not tex_path.exists():
            print(f"Error: Texture file not found at {tex_path}")
            sys.exit(1)

        print(f"Rendering new version for block '{args.block}'...")
        version_dir = create_new_version(workspace_root, args.block, tex_path, config)
        ...
        for view in views:
            ...
            render_isometric_block(
                texture_path=str(version_dir / "texture.png"),
